...
`.agents/skills/` is the agent-neutral home for project skills. Write there only; if the repo links agent-specific directories (such as `.claude/skills/`) to it, add a matching relative symlink for `verify-<app>` so every agent discovers the same files. Never write a second copy.
